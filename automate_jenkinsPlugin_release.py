...
def clean_workspace_and_exit(error_msg):
    if (error_msg != ""):
        print(error_msg)
...
try:
    matlab_repo = git.Repo.clone_from(sourceGithub_url, "workspace\Matlab Repo")
except:
    clean_workspace_and_exit(
        "Error while cloning from remote matlab (i.e.matlab github).\nPlease check you sourceGithub url tag value in your config.xml file.")
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
...
try:
    jenkins_repo = git.Repo.clone_from(targetGithub_url, "workspace\Jenkins Repo")
except:
    clean_workspace_and_exit(
        "Error while cloning from remote jenkins (i.e.jenkins github).\nPlease check you targetGithub url tag value in your config.xml file.")
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
...
try:
    ...
    clean_workspace_and_exit(
        "Error while connecting to jenkins server.\nPlease check your jenkinsServer tag values in your config.xml file.")
...
try:
    ...
    clean_workspace_and_exit(
        "Error while connecting to jenkins server.\nPlease check your jenkinsServer projectName tag values in your config.xml file.")
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
...
try:
    ...
    print("Error while fetching user.name or user.password form your git config file.")
...
while (not (validate_username_password)):
    ...
    if (not (have_access)):
        ...
        if (another_credential.lower() == 'n'):
            clean_workspace_and_exit("Failed to validate credentials.")
...
while (change_tags):
    ...
    if (confirmation_flag == 'q'):
        clean_workspace_and_exit("process aborted...")
...
if(release_version != "test"):
    ...
    subprocess.os.chdir("workspace\Jenkins Repo")
